...
<p class="intro">Use this element to create tab menus that switch the content in pre-defined regions on a page.</p>
...
<script>
    $('#exampleButton-1').on('click', function() {
        YOI.element.Tabs.switchTo('#tab-8-2');
    });
</script>
```
...
<script>
    $('#exampleTabs-2').on('yoi-tabs:change', function() {
        YOI.element.Log.write($('#myLog'), 'yoi-tabs:change');
    });
</script>
```